...
class Arduino2LightInterface(object):
	...
	def __init__(self, com_port=None, default_on=False, read_from_queue=True):
		"""
		...
		if com_port is None:
			if sys.platform.lower().startswith('win'):
				raise ValueError('must provide com port for Windows')
				pass
			else:
				devs = listdir('/dev/')
				for dev in devs:
		...
		elif type(com_port) is int:
			if sys.platform.lower().startswith('win'):
				com_port = 'COM%d' % com_port
			else:
				com_port = '/dev/cu.usbmodem%i' % com_port
...
if __name__ == '__main__':
	'''Example Use Case'''
	ard = Arduino2LightInterface(default_on=False)
	ard.turn_both_on()
	time.sleep(2)
	ard.turn_both_off()
	time.sleep(2)
